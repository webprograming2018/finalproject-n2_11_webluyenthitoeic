...
<div class="main">
	...
	<div class="col-md-8 colQuestion">
        ...
        <div id="fulltest_content">
            <form method="post" id="fulltest_part_head" action="<%=request.getContextPath()%>/pratice/result" data-part="part7">
                <input type="hidden" value="part7" name="part"/>
                <div  data-media="media_<%=pass.getId()%>"  class="question fullest_page_<%=index+1%> part3_<%=pass.getId()%>" data-page="<%=index+1%>"  data-id="<%=pass.getId()%>" data-part="part7">
                        <div class="text-center" style="background: #00a7d0; padding: 20px; color:18px">
                                <%=pass.getContent()%>
                        </div>
                </div>
                    <% 
                        ArrayList<Part7> getPart = passages.getPart7(String.valueOf(pass.getId())) ;
                        for(Part7 p1: getPart){
                    %>
                    <div  class="question  media_<%=p1.getPasages_id()%>" data-page="<%=dem+1%>"  id="test_question_<%=p1.getId()%>" data-part="3">
                        <p>Question: <%=dem+1%> </p>
                        <ol id="que" type="A"  >
                            <li>
                                <input type="radio" data-cau="<%=dem+1%>" data-part="1" data-question="<%=p1.getId()%>" name="answer[<%=dem%>]" value="<%=p1.getId()%>" class="sg-replace-icons radio_answer_a<%=p1.getId()%><%=p1.getPasages_id()%>">
                                <label for=""><%=p1.getA()%></label>
                            </li>
                            <li>
                                <input type="radio" data-cau="<%=dem+1%>" data-part="1" data-question="<%=p1.getId()%>" name="answer[<%=dem%>]" value="<%=p1.getId()%>" class="sg-replace-icons radio_answer_b<%=p1.getId()%><%=p1.getPasages_id()%>"> 
                                <label><%=p1.getB()%>
                                </label>
                            </li>
                            <li>
                                <input type="radio" data-cau="<%=dem+1%>" data-part="1" data-question="<%=p1.getId()%>" name="answer[<%=dem%>]" value="<%=p1.getId()%>" class="sg-replace-icons radio_answer_c<%=p1.getPasages_id()%>"> 
                                <label><%=p1.getC()%>
                                </label>
                            </li>
                            <li>
                                <input type="radio" data-cau="<%=dem+1%>" data-part="1" data-question="<%=p1.getId()%>" name="answer[<%=dem%>]" value="<%=p1.getId()%>" class="sg-replace-icons radio_answer_d<%=p1.getId()%><%=p1.getPasages_id()%>"> 
                                <label><%=p1.getD()%>
                                </label>
                            </li>
                        </ol>
                ...
                <%
                        for (int i = 1; i < listPassages.size(); i++) {
                            ...
                            index++;
                %>
                <div  data-media="media_<%=passage.getId()%> " style="display:none" class="question fullest_page_<%=index+1%> part3_<%=passage.getId()%>" data-page="<%=index+1%>"  data-id="<%=passage.getId()%>" data-part="part7">
                        <div class="text-center" style="background: #00a7d0; padding: 20px; color:18px">
                                <%=passage.getContent()%>
                        </div>
                    ...
                    <% 
                        for (int idx = dem; idx < listPart6.size(); idx++) {
                            Part7   p1 = listPart6.get(idx);
                            dem++;
                    %>
                    <div  style="display:none" class="question  media_<%=p1.getPasages_id()%>" data-page="<%=dem+1%>"  id="test_question_<%=p1.getId()%>" data-part="3">
                        <p>Question: <%=dem+1%> </p>
                        <ol id="que" type="A"  >
                            <li>
                                <input type="radio" data-cau="<%=dem+1%>" data-part="1" data-question="<%=p1.getId()%>" name="answer[<%=dem%>]" value="<%=p1.getId()%>" class="sg-replace-icons radio_answer_a<%=p1.getId()%><%=p1.getPasages_id()%>">
                            ...
                            <li>
                                <input type="radio" data-cau="<%=dem+1%>" data-part="1" data-question="<%=p1.getId()%>" name="answer[<%=dem%>]" value="<%=p1.getId()%>" class="sg-replace-icons radio_answer_c<%=p1.getId()%><%=p1.getPasages_id()%>"> 
                                <label><%=p1.getC()%>
                                </label>
                            </li>
                            <li>
